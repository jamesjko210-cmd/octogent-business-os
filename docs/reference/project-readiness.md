...
| GitHub history | Waiting for operator input | The current upstream is deliberately blocked. A token-free, user-owned remote and explicit approval are required before any configuration, commit, or push; the [publication checklist](../guides/user-owned-github-publication.md) is ready locally. |
...
1. Choose a user-owned GitHub repository if version publishing is wanted.
2. Create a dedicated Telegram bot and configure its token plus trusted chat IDs outside the repository if mobile operator access is wanted.
3. Choose one provider integration to pilot, with no API keys stored in Octogent by default.

## Verification baseline

The current local regression baseline is 6 core tests, 227 API tests, and 118 web tests. Core, API, and web TypeScript checks plus a full Biome check and `git diff --check` passed. The web production build passed with the existing non-blocking large-chunk warning.
